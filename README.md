...
- 多服务部署：Docker Compose 当前收敛为 v2-only，启动 Vue、Java、MySQL 和 FastAPI。
...
当前 `codex/v2-only-remove-v1` 分支正在准备从双轨切换到 v2-only。切换前的运行逻辑和删除候选见 `docs/V2_ONLY_RUNTIME_SIMPLIFICATION.md`；在用户确认前，v1 入口仍保留。
...
- Figma-first UI 设计源：为 Vue 前端维护个人可编辑的 Figma 设计源、页面映射和组件清单。
...
- Figma Variables / Component Variants 与 Vue 组件持续同步。
...
- Figma：v2 Vue 前端的可编辑设计源、组件映射和后续 UI 修改入口。
...
项目已新增 `figma/` 目录，用于把 v2 Vue 前端转向 Figma-first 工作流：

- `figma/design_tokens.json`：颜色、字体、间距、圆角等设计 token。
- `figma/frontend_ui_map.json`：Vue 页面到 Figma Frame 的映射。
- `figma/component_inventory.json`：Vue 组件到 Figma Component 的映射。
- `figma/create_figma_ui_source.js`：可通过 Figma `use_figma` 执行的初始化脚本。

后续重要前端 UI 修改应优先考虑：先更新 Figma 可编辑设计源，再同步到 Vue 实现。详细流程见 `docs/FIGMA_UI_WORKFLOW.md`。
...
如果本机已有 MySQL 占用 `3306`，可临时改宿主端口再启动：

...
当前编辑器在 Python Direct 模式下保存到浏览器 localStorage；在 Java Gateway 模式下可额外保存到 MySQL，接口为 `POST /api/platform/workflows/templates`。MySQL 模板支持详情预览、覆盖保存版本递增、删除，以及一键生成可回放任务视图。生成任务时仍不会直接改写 LangGraph 主流程。

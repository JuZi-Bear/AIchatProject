...
当前 `codex/v2-only-remove-v1` 分支用于准备去除 v1 演示轨。执行删除或归档前，必须先参考 `docs/V2_ONLY_RUNTIME_SIMPLIFICATION.md`，确认 Vue -> Java -> Python -> MySQL/SSE/Replay 主链路不受影响。
...
- `figma/` 是 v2 Vue 前端的 Figma-first 设计源目录，后续重要 UI 修改应优先考虑 Figma 可编辑设计源与 Vue 实现同步。
...
- 后续涉及 `frontend-vue/` UI 创建或修改时，必须先参考 `docs/FIGMA_UI_WORKFLOW.md`、`figma/frontend_ui_map.json` 和 `figma/component_inventory.json`。
- 如果用户提供了 Figma 文件 URL，应优先同步 Figma Frame / Component，再修改 Vue 实现。
- 如果暂时没有 Figma 文件 URL，应至少更新 `figma/` 下的设计源规范文件，避免 UI 只沉淀在代码里。
- v2-only 分支上不要直接删除 v1 文件；先按 `docs/V2_ONLY_RUNTIME_SIMPLIFICATION.md` 的决策点确认“删除”还是“移动到 legacy”。

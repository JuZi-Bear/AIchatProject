...
本文用于说明当前项目目录结构、目录职责和双轨归属。目录树省略了 `.venv/`、`node_modules/`、`target/`、`build/`、`dist/`、缓存目录等大型或生成目录。
...
│  └─ ui_workflow/          documentation，UI 工作流资料
...
├─ webui.py                 v1-demo，Streamlit 演示入口
├─ graph_demo.py            v1-demo，CLI 演示入口
├─ main.py                  v1-demo/shared-core，命令行入口
...
| 分类 | 含义 |
| --- | --- |
| `v1-demo` | 比赛现场稳定演示链路，优先保证可运行、可兜底 |
| `v2-platform` | 平台化升级链路，包含 API、Vue、Java、MySQL、Docker 等 |
| `shared-core` | v1 和 v2 共享的 Agent 核心、插件、配置和工具 |
...
插件系统实现目录，包含 Doc Agent、Security Agent、Refactor Agent、UI Agent 等插件。v1 和 v2 都依赖插件结果展示。

### utils/：shared-core

工具函数目录，包含代码执行、C++ Runner 适配、历史存储、摘要构建、UI ViewModel 构建等。修改时要同时考虑 Streamlit、FastAPI、Java 和 Vue 展示链路。
...
Python 运行历史 JSON 目录。v1 和 Python Direct 模式仍依赖该目录读取历史。

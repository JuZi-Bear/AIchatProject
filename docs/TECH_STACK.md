...
- `PlatformWorkflowController` / `WorkflowTemplateService` / `WorkflowTemplateEntity`：提供 `/api/platform/workflows/templates`，将 Vue Workflow Editor 自定义模板保存到 MySQL，并支持详情查询、覆盖保存版本递增、删除和生成可回放平台任务。

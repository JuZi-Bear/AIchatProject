...
| `java` | Java Gateway | `VITE_JAVA_API_BASE_URL` | `/agent/health` | `/models`、`/plugins`、`/agents`、`/workflows/templates`、`/workflows/instantiate`、`/runs`、`/reports`、`/settings`、`/platform/runs`、`/platform/events/recent` |
...
## POST /api/workflows/templates/save（预留）

该接口是后续后端持久化自定义 Workflow 模板的预留契约，当前未落地到 Python/Java/MySQL。当前 Vue Workflow Editor 的“保存模板”先保存到浏览器 localStorage，并支持导出 JSON。
...
```json
{
  ...
  "agent_sequence": ["product", "coder", "tester", "runner", "sentry", "coder", "quality", "report"],
  "stage_sequence": ["分析", "生成", "测试", "执行", "修复", "再生成", "评分", "报告"]
}
```

...
| --- | --- | --- | --- | --- |
| `python` | Python Direct | `VITE_PYTHON_API_BASE_URL` | `/health` | `/models`、`/plugins`、`/agents`、`/api/workflows/templates`、`/api/workflows/instantiate`、`/api/workflows/dynamic/*`、`/runs`、`/reports` |
| `java` | Java Gateway | `VITE_JAVA_API_BASE_URL` | `/agent/health` | `/models`、`/plugins`、`/agents`、`/workflows/templates`、`/workflows/instantiate`、`/platform/workflows/templates`、`/platform/workflows/templates/{key}/execute-langgraph`、`/platform/workspaces`、`/runs`、`/reports`、`/settings`、`/platform/runs`、`/platform/events/recent` |
...
Java Gateway 模式下执行 MySQL 中保存的 Workflow 模板。该接口属于平台层演示执行器：它会读取模板节点与连线，按拓扑顺序记录 RunEvent 并通过 SSE 推送；`code_agent` 节点会调用现有 Python CodeAgent 执行文件操作，`human_approval` 节点会让任务进入 `WAITING_FOR_HUMAN`，`report` / `report_generator` 节点会生成 Runtime Lite Markdown 演示报告，其它 Agent 节点第一版作为 `simulated` 平台事件，不动态改写 LangGraph。
...
```json
{
  ...
  "data": {
    ...
    "run_summary": {
      ...
      "report_path": "reports/workflow_runtime_1780000000000_runtime_lite_report.md",
      "runtime_node_counts": {
        "executed": 2,
        "simulated": 2,
        "waiting": 1
...
- `executed`：真实执行节点，目前包括 CodeAgent 和 Runtime Lite 报告生成。
...
## Dynamic LangGraph Runtime API

Dynamic LangGraph Runtime v1 是 Python Agent Engine 的受控执行图入口。它不会替换固定 `/runs` 主流程，也不会接受任意 Python 代码或任意表达式；模板必须先通过校验，条件表达式只能使用白名单字段和比较操作，循环必须配置 `maxIterations`。

### POST `/api/workflows/dynamic/validate`

校验前端 Workflow Editor 保存的模板 JSON 是否可以转换为受控 LangGraph 执行图。

请求：

```json
{
  "template_key": "repair_with_approval",
  "template": {
    "nodes": [],
    "connections": []
  }
...
```json
{
  ...
  "validation_result": {
    ...
    "errors": [],
    "warnings": [],
    "entry_nodes": ["product_1"],
    "terminal_nodes": ["report_1"]
  }
}
```

校验会检查入口节点、终止路径、缺失节点、字段映射、条件表达式、循环上限和暂停节点。

### POST `/api/workflows/dynamic/execute`

执行一个已校验的动态 Workflow 模板。第一版支持 Product / Coder / Tester / Runner / Sentry / Quality / Report 节点映射到现有 Python 节点，CodeAgent 调用受控文件操作能力，Human Approval 会暂停到 `WAITING_FOR_HUMAN`。
...
{
  "template_key": "repair_with_approval",
  "platformRunId": "dynamic_langgraph_1780000000000",
  "input_data": {
    "requirement": "生成一个可测试函数，并在失败时进入修复循环"
  },
  "template": {
    "nodes": [],
    "connections": []
...
```json
{
  "success": true,
  "platformRunId": "dynamic_langgraph_1780000000000",
  "run_id": "dynamic_langgraph_1780000000000",
  "status": "WAITING_FOR_HUMAN",
  "validation_result": {
    "valid": true,
    "errors": [],
  ...
  "workflow_events": [],
  "run_summary": {
    "runner_mode": "dynamic_langgraph",
    "runtime_mode": "dynamic_langgraph",
    "require_human_approval": true,
    "report_path": "reports/dynamic_langgraph_1780000000000_dynamic_langgraph_report.md"
  },
  "ui_view_model": {
    "workflow_events": [],
    "runtime_summary": {
...
### POST `/api/workflows/dynamic/runs/{run_id}/resume`

恢复一个暂停中的动态运行。第一版通过平台保存的 dynamic state 恢复，不依赖复杂 checkpoint；已完成节点不会重复执行。
...
{
  "approved": true,
  "comment": "人工确认通过，继续执行 Report 节点"
}
```

响应结构与 execute 保持兼容，`status` 通常变为 `SUCCESS` 或 `REJECTED`。

## Java Platform Dynamic LangGraph Proxy API

Java Gateway 模式下，Vue 不直接读取 MySQL 模板再调用 Python，而是通过 Java 平台接口完成模板读取、Python 代理、RunRecord 保存、RunEvent 入库和 SSE 推送。

### POST `/api/platform/workflows/templates/{templateKey}/validate-langgraph`

从 MySQL 读取指定 Workflow 模板，代理 Python `POST /api/workflows/dynamic/validate`，返回 `ApiResponse` 包装后的校验结果。

### POST `/api/platform/workflows/templates/{templateKey}/execute-langgraph`

从 MySQL 读取模板并代理 Python 动态执行。Java 会生成 `platformRunId`，保存 `run_summary`、`ui_view_model`、`workflow_events` 和报告索引；Python 返回的每条 `workflow_events` 会保存为 Java RunEvent 并通过 SSE 推送。
...
{
  "input_data": {
    "requirement": "执行动态 LangGraph 模板"
  }
}
```

### POST `/api/platform/workflows/runs/{platformRunId}/resume`

恢复一个 Java 平台记录对应的动态 LangGraph 运行。Java 根据 `platformRunId` 找到 Python `run_id`，代理 Python resume 接口，并追加保存新的事件。
...
- 不修改现有 `/runs` 固定工作流。
- 不替代 Java Workflow Runtime Lite；Lite 仍用于平台层演示执行器。
- 不支持前端提交任意 Python 代码。
- 分支条件只允许安全字段比较；循环边必须有最大次数。
- 字段级连线进入模板、校验和回放展示，真实字段值传递仍是后续深化方向。

## Platform Workflow Skill Export API

### POST `/api/platform/workflows/templates/{templateKey}/export-skill`

Java Gateway 模式下将 MySQL Workflow 模板导出为本地 Codex Skill 包。该接口只生成文件，不自动安装到 Codex，也不会直接调用 Python、模型或 CodeAgent 文件系统。
...
```json
{
  ...
  "message": "ok",
  "data": {
    "templateKey": "demo_flow",
    "skillName": "demo-flow",
    "skillPath": "generated-skills/demo-flow",
    "files": [
      "SKILL.md",
      "references/workflow-template.json",
      "scripts/run_workflow.py"
    ...
    "installed": false,
    "warnings": ["Skill 已导出但未自动安装到 Codex。"]
  }
}
```
...
- `SKILL.md` 只包含 `name` 和 `description` frontmatter。
- `references/workflow-template.json` 保存完整模板 JSON。
- `scripts/run_workflow.py` 默认调用 `http://127.0.0.1:8088/api/platform/workflows/templates/{templateKey}/execute-langgraph`。
- 可通过 `--api-base` 和 `--requirement` 参数改变 Java Gateway 地址和任务需求。
- Docker 模式下 `backend-java` 将 `./generated-skills` 挂载为 `/app/generated-skills`。
